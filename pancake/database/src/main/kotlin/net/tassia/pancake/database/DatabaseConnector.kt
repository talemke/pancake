...
object DatabaseConnector {
	...
	private fun connectToMySQL(config: PancakeConfig): Database {
		...
		val url = MYSQL_URL.format(config.databaseMySQLHostname, config.databaseMySQLPort, config.databaseMySQLDatabase)
		return Database.connect(url, MYSQL_DRIVER, config.databaseMySQLUsername, config.databaseMySQLPassword)
	}
