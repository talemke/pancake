...
object DefaultFormatter : LogFormatter<Plugin> {
	...
	override fun format(record: LogRecord<Plugin>): String {
		val message = record.message?.let {
			formatLines(record.timestamp, record.level, it, record.data)
		} ?: ""
		...
		val error = formatLines(record.timestamp, record.level, stackTrace, record.data)
