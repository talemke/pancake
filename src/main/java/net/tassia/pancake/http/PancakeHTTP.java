...
public class PancakeHTTP {
	...
	public PancakeHTTP(Pancake pancake) throws IOException {
		...
		this.server = HttpServer.create(new InetSocketAddress(pancake.getConfig().httpPort), pancake.getConfig().httpBacklog);
		...
		server.createContext("/", (exchange) -> {
			...
			dispatchRequest(req);
			try {
				req.setResponseHeader("Content-Type", req.getContentType());
				req.setResponseHeader("Accept-Charset", "utf-8");
