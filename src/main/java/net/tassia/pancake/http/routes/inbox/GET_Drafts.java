...
class GET_Drafts implements HttpRoute {
	private final InboxRoutes routes;
	private final String noneView;

	public GET_Drafts(InboxRoutes routes) {
		this.routes = routes;
	...
	public byte[] route(Pancake pancake, HttpRequest request, String[] matches) {
		...
		Collection<Mail> mail = pancake.getHTTP().getResources().findDraftEmails(request, request.getAuth(), 0, 0);
